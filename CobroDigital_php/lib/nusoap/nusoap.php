...
class nusoap_server extends nusoap_base {
	...
	*/
	function send_response() {
		...
		if ($this->fault) {
			...
			$this->outgoing_headers[] = "Status: 5000 Internal Server Error";
		} else {
  ...
  function verify_instru() {
    $hret=array();
    foreach ($this->operations[$this->methodname]['in'] AS $kdesired=>$vdesired) {
     foreach ((array)$this->methodparams AS $kpassed=>$vpassed) {
      if ($kpassed === $kdesired) {
    	$this->debug('Parameter ' . $kpassed . ' Accepted');
       switch ($vdesired) {
        case 'xsd:string':
         $hret[$kpassed]=(string)$vpassed;
         break;
        case 'xsd:integer':
         $hret[$kpassed]=(integer)$vpassed;
         break;
        case 'xsd:array':
         $hret[$kpassed]=(array)$vpassed;
         break;
        default:
         $hret[$kpassed]=$vpassed;
         break;
       }
      }
     }
    }
    $this->valid_params=$hret;
    //return $hret;
	}
	...
	* @access   public
	*/
	function register($name,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,$documentation='',$encodingStyle='',$otherinfo=''){
		global $HTTP_SERVER_VARS;
...
class wsdl extends nusoap_base {
    ...
    */
    function webDescription(){
		...
		return $b;
    return 'Solicitar el manual de este servicio
            <p>View the <a href="'.$PHP_SELF.'?wsdl">WSDL</a> for the service.
    				Click on an operation name to view it&apos;s details.</p>';
		return $b;
    }
